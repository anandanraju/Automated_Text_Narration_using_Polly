...
def lambda_handler(event, context):
    try:
        ...
        # Read text file from text-files-bucket-polly
        obj = s3.get_object(Bucket="text-files-bucket-polly", Key=file_key)
        ...
        # Upload audio to audio-files-bucket-polly
        s3.put_object(
            Bucket="audio-files-bucket-polly",
            Key=audio_key,
            Body=audio_bytes,
